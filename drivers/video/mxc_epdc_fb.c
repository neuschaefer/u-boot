...
extern int setup_waveform_file( unsigned char*pucTrt, ulong* pulTrtSize);
extern void epdc_power_on(void);
extern void epdc_power_off(void);
/* 2011/04/05 FY11 : Supported to get temperature. */
extern int get_temperature( int *pTemp );
/* 2011/04/12 FY11 : Supported to read and write panel init flag. */
extern int get_epd_setting_flag( unsigned long *pulFlag );
extern int set_epd_setting_flag( unsigned long ulFlag );
...
/* 2011/04/05 FY11 : Modified default value to degree from index. */
#define TEMP_USE_DEFAULT 25
...
/* 2011/05/06 FY11 : Fixed no flash update in despite of FULL update mode. */
static void epdc_submit_update(u32 lut_num, u32 waveform_mode, u32 update_mode,
			       int use_np, u32 np_val, int use_cp, u32 cp_val)
{
	u32 reg_val = 0;
	...
	if ( use_np | use_cp ) {
		if ( use_np )
		{ 
			reg_val |= EPDC_UPD_FIXED_FIXNP_EN |
				( (np_val << EPDC_UPD_FIXED_FIXNP_OFFSET) & EPDC_UPD_FIXED_FIXNP_MASK) ; 
		}
		else if ( use_cp )
		{
			reg_val |= EPDC_UPD_FIXED_FIXCP_EN |
				( (cp_val << EPDC_UPD_FIXED_FIXCP_OFFSET) & EPDC_UPD_FIXED_FIXCP_MASK) ;
		}
...
/* 2011/04/05 FY11 : Disabled to use default temperature index. */
...
//	REG_WR(EPDC_BASE, EPDC_TEMP, TEMP_USE_DEFAULT);
	...
	/* EPDC_TCE_TIMING2 */
	reg_val =
		((465 << EPDC_TCE_TIMING2_GDCLK_HP_OFFSET) &
	...
	reg_val =
		...
		| ((9 << EPDC_TCE_TIMING3_GDCLK_OFFSET_OFFSET) &
...
/* 2011/04/05 FY11 : Supported to read temperature and set index. */
static int set_temperature_index(void)
{
	int ret = 0, i=0;
	int temperature=0;
	...
	if ( ret < 0 )
	{
		printf( "Use default temp index.\n" );
		temperature = TEMP_USE_DEFAULT;
	}

	printf( "temperature %d\n", temperature );

	if ( panel_info.epdc_data.trt_size > 0 )
	{
		/* select temperature index. */
		/* if temperature is lower than minimum, use index 0. */
		/* if temperature is higher than max, use index max. */
		for ( i = 0; i < (panel_info.epdc_data.trt_size - 1); i++ )
		{
			if ( temperature < (signed int)(panel_info.epdc_data.trt[i+1]) )
			{
				break;
			}
		}
...
/* 2011/05/06 FY11 : Fixed no flash update in despite of FULL update mode. */
	...
	debug("Splash screen update started!\n");
...
/* 2011/04/05 FY11 : Supported power-up and temperature reading. */
	epdc_power_on();
	set_temperature_index();

/* 2011/04/12 FY11 : Supported to read and write panel init flag. */
	get_epd_setting_flag( &ulInitFlag );
	if ( ulInitFlag & EPD_SETTING_PANEL_INIT )
	{
		/* initialize panel only when panel init flag is set. */
		draw_mode0();
	}

/* 2011/04/12 FY11 : Supported to read and write panel init flag. */
	ulInitFlag |= EPD_SETTING_PANEL_INIT;
	set_epd_setting_flag( ulInitFlag );

	/* draw bootup image */
	draw_splash_screen();
}

/* 2011/04/05 FY11 : Added wait completion of updaing panel. */
...
/* 2011/06/06 FY11 : Turned off EPD after complete updating panel. */
			epdc_power_off();
			return;
		}
		msleep(100);
	}
	printf("Splash screen update timeout !!!\n" );
...
/* 2011/04/05 FY11 : Supporte temperature range table. */
	if (setup_waveform_file( &(panel_info.epdc_data.trt[0]), &(panel_info.epdc_data.trt_size))) {
