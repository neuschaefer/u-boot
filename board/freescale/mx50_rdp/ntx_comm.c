...
#define SD_OFFSET_SECS_LOGO_POWERLOW	19456 //angela 20121011 add power low
#define SD_OFFSET_SECS_LOGO2			45056
#define SD_OFFSET_SECS_LOGO_POWERLOW2	46080 //Terry 20121011 add power low2
...
NTX_HWCONFIG gtNtxHwCfg,*gptNtxHwCfg=0,*gptEsdNtxHwCfg=0;
...
extern void frontLightCtl(void);
...
const static char gszKParamName_logo_powerlow[]="logo_powerlow"; //angela 20121011 add power low
...
static void _load_ntxbins_and_append_boot_args(char *O_cBufA,unsigned long I_ulBufSize)
{
	...
	// [ WARNING : be pair .
	int iLoadSecNO[] = {SD_OFFSET_SECS_HWCFG,SD_OFFSET_SECS_WAVEFORM,SD_OFFSET_SECS_LOGO,SD_OFFSET_SECS_LOGO2,SD_OFFSET_SECS_LOGO_POWERLOW,SD_OFFSET_SECS_LOGO_POWERLOW2};
	char *szLoadNameA[] = {(char *)gszKParamName_HwCfg,(char *)gszKParamName_waveform,(char *)gszKParamName_logo,(char *)gszKParamName_logo,(char*)gszKParamName_logo_powerlow,(char*)gszKParamName_logo_powerlow};
	// ] 
	...
	for(i=0;i<sizeof(iLoadSecNO)/sizeof(iLoadSecNO[0]);i++)
	{
			...
			&&gszNtxBinMagicA[2]==pbMagic[2]&&gszNtxBinMagicA[3]==pbMagic[3])
		{
			...
			if(0==strcmp(szLoadNameA[i],gszKParamName_HwCfg)) {
				if(iLoadDeviceNum==GET_ESD_NUM()) {
					gptEsdNtxHwCfg = pbLoadAddr;
					_load_isd_hwconfig();
				}
				else {
			...
			if(iLoadDeviceNum==GET_ESD_NUM() && 0==strcmp(szLoadNameA[i],gszKParamName_HwCfg)) {
				printf("no esd hwcfg header , use isd hwcfgp=%p\n",gptNtxHwCfg);
				if(gptNtxHwCfg) {
					//dwReadBytes=gdwNtxHwCfgSize;
					dwReadBytes=(gdwNtxHwCfgSize>>9)<<9;
					if(gdwNtxHwCfgSize&0x1ff) {
						gdwNtxHwCfgSize += 512;
					}
					pbLoadAddr -= dwReadBytes;
...
static void _load_ntxkernel(void)
{
		...
		&&gszNtxBinMagicA[2]==pbMagic[2]&&gszNtxBinMagicA[3]==pbMagic[3])
	{
		...
		dwReadSectors += 6;
		//dwReadSectors = 0xfff;
		sprintf(cCmdA,"mmc read %d 0x%x 0x%x 0x%x",gi_mmc_num_kernel,KERNEL_RAM_ADDR,\
...
#if 0	// diable boot from linux recovery/ESD .
...
void ntx_prebootm(void)
{
	...
	if(pstr && gptNtxHwCfg) {
		if(gptEsdNtxHwCfg) {
			...
			bUIStyle = gptEsdNtxHwCfg->m_val.bUIStyle;
		}
		else {
		...
		if(2==bUIStyle)
		{
			...
			strcat(gszBootArgs,&gbSectorBufA[3]);
			//printf("bootargs=\"%s\"\n",gszBootArgs);
		}
		else {
			// linux UI .
