...
static struct pin_config sdio4_config[] = {
    ...
    PIN_CFG(GPIO14, VC_GPIO_5, 0, OFF, OFF, 0, 0, 8MA),
...
static struct pin_config clockout_config[] = {
    PIN_CFG(CLKREQ_IN_0, CLKREQ_IN_0, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CLKREQ_IN_0, CLKREQ_IN_0, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CLKOUT_0, CLKOUT_0, 0, OFF, OFF, 0, 1, 8MA),
    PIN_CFG(CLKOUT_1, GPIO_180, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CLKOUT_2, VC_GPCLK_0, 0, OFF, OFF, 0, 1, 8MA),
    PIN_CFG(CLKOUT_3, GPIO_182, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CWS_SYS_REQ1, GPIO_183, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CWS_SYS_REQ2, GPIO_184, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(CWS_SYS_REQ3, GPIO_185, 0, ON, OFF, 0, 0, 2MA),
};
...
static struct pin_config ungrouped_pins_config[] = {
    ...
    // instead, RGMII_GPIO_2 and RGMII_GPIO_3 are used as MDIO/MDC
    // the MII stardard suggested to pull up MDIO/MDC, on 11130 Ray board the 2 signals are pulled up.
    // Roku board, there is no such pull-up, so better to configure it internally pulled up.
    PIN_RGMII_CFG(RGMII_GPIO_2, MDIO, 0x20),
    PIN_RGMII_CFG(RGMII_GPIO_3, MDC, 0x20),

	// Roku2 EVT/DVT GPIO_2:0 are used as Board ID
    PIN_CFG(GPIO00, GPIO_000, 0, OFF, ON,  0, 0, 8MA),
    PIN_CFG(GPIO01, GPIO_001, 0, OFF, ON,  0, 0, 8MA),
    PIN_CFG(GPIO02, GPIO_002, 0, OFF, ON,  0, 0, 8MA),

	// Unsed GPIOs
    PIN_CFG(GPIO03, GPIO_003, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(GPIO04, GPIO_004, 0, ON,  OFF, 0, 0, 2MA),
	// GPIO5 Reset Button
    PIN_CFG(GPIO05, GPIO_005, 0, OFF, OFF, 0, 0, 8MA),
    PIN_CFG(GPIO06, GPIO_006, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(GPIO07, GPIO_007, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(GPIO08, GPIO_008, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(GPIO09, GPIO_009, 0, ON,  OFF, 0, 0, 2MA),

    // GPIO 10 is used as PHY RST
    PIN_CFG(GPIO10, GPIO_010, 0, OFF, OFF,0, 0, 8MA),
    // GPIO 11 is used as WiFi RST
    PIN_CFG(GPIO11, GPIO_011, 0, OFF, ON, 0, 0, 8MA),
    // Roku Austin: GPIO 12 - LED 
    PIN_CFG(GPIO12, GPIO_012, 0, OFF, ON, 0, 0, 8MA),
    // GPIO 13 is used as Wake_On_WiFi
    PIN_CFG(GPIO13, GPIO_013, 0, OFF, ON, 0, 0, 8MA),
    // GPIO 14 is used as SDTV Reset
    PIN_CFG(GPIO14, VC_GPIO_5,0, OFF, ON, 0, 0, 8MA),


    PIN_CFG(KP_ROW_OP_0, GPIO_057,  0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_ROW_OP_1, GPIO_058,  0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_ROW_OP_2, GPIO_059,  0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_ROW_OP_3, GPIO_060,  0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_COL_IP_0, GPIO_061,  0, ON,  OFF, 0, 0, 2MA),
	// Roku2 COL_IP_1/VC_GPIO_3 used as HDMI HPD
    PIN_CFG(KP_COL_IP_1, VC_GPIO_3, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_COL_IP_2, GPIO_063,  0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(KP_COL_IP_3, GPIO_064,  0, ON,  OFF, 0, 0, 2MA),

    PIN_CFG(CLKREQ_IN_1, PWM_O_0, 0, OFF, OFF, 1, 1, 8MA),
    PIN_CFG(MTX_SCAN_CLK, GPIO_175, 1, ON, OFF, 0, 0, 8MA),

	// Power Changes
 	// Roku uses LCD_B, HSYNC, VSYNC and PCLK for SDTV output ( see lcd_config )
	// rest are configured as input for power savings
    PIN_CFG(LCD_R_7, GPIO_146, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_6, GPIO_147, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_5, GPIO_148, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_4, GPIO_149, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_3, GPIO_150, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_2, GPIO_151, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_1, GPIO_152, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_R_0, GPIO_153, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_7, GPIO_154, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_6, GPIO_155, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_5, GPIO_156, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_4, GPIO_157, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_3, GPIO_158, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_2, GPIO_159, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_1, GPIO_160, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_G_0, GPIO_161, 0, ON,  OFF, 0, 0, 2MA),
    PIN_CFG(LCD_OE,  GPIO_172, 0, ON,  OFF, 0, 0, 2MA),

	// Roku uses RUN1:CIR_RX, HCE1:Strap7, DATA_8:SD_Detect
    PIN_CFG(MPHI_DATA_10, 	GPIO_121, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_RUN0, 		GPIO_122, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_9, 	GPIO_123, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_HCE0_N, 	GPIO_124, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_HWR_N, 	GPIO_125, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_HRD_N, 	GPIO_126, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_HA0,	 	GPIO_127, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_HAT0, 		GPIO_128, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_8, 	GPIO_129, 0, OFF, ON, 0, 0, 8MA),	// SD Card Detect
// tied to data_8, make the same
    PIN_CFG(MPHI_DATA_6,    GPIO_139, 0, OFF, ON, 0, 0, 8MA),

    PIN_CFG(MPHI_DATA_15, 	GPIO_130, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_14, 	GPIO_131, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_13, 	GPIO_132, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_12, 	GPIO_133, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_11, 	GPIO_134, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_RUN1,		CIR_RX,   1, OFF,OFF, 0, 0, 8MA),	// Disable Pulldown due to DVT platform
    PIN_CFG(MPHI_HCE1_N,	GPIO_136, 0, OFF,OFF, 0, 0, 8MA),	// Strap 7
    PIN_CFG(MPHI_HAT1, 		GPIO_137, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_7, 	GPIO_138, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_5,    GPIO_140, 0, ON, OFF, 0, 0, 2MA),
// tied to RUN1, make the same
    PIN_CFG(MPHI_DATA_4,    GPIO_141, 1, OFF,OFF, 0, 0, 8MA),
    PIN_CFG(MPHI_DATA_3,    GPIO_142, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_2,    GPIO_143, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(MPHI_DATA_1,    GPIO_144, 0, ON, OFF, 0, 0, 2MA),
// tied to RUN1, make the same
    PIN_CFG(MPHI_DATA_0, 	GPIO_145, 1, OFF,OFF, 0, 0, 8MA),	

    PIN_CFG(UARTB1_URTS, UARTB1_URTS, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB1_UCTS, UARTB1_UCTS, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(UARTB3_URTS,    GPIO_075, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB3_UCTS,    GPIO_076, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB3_URXD,    GPIO_077, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB3_UTXD,    GPIO_078, 0, ON, OFF, 0, 0, 2MA),
 
    PIN_CFG(UARTB4_URTS,    GPIO_079, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB4_UCTS,    GPIO_080, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB4_URXD,    GPIO_081, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(UARTB4_UTXD,    GPIO_082, 0, ON, OFF, 0, 0, 2MA),

	// Roku uses TXD:Strap8, RXD_1:AMPShutdown
    PIN_CFG(SSP2_FS_0,      GPIO_095, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_FS_1,      GPIO_096, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_FS_2,      GPIO_097, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_FS_3,      GPIO_098, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_CLK,       GPIO_099, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_RXD_0,     GPIO_100, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP2_TXD_0,     GPIO_101, 0, OFF,OFF, 0, 0, 8MA),	// Strap 8
    PIN_CFG(SSP2_RXD_1,     GPIO_102, 0, ON, OFF, 0, 0, 8MA),	// AMP Shutdown
    PIN_CFG(SSP2_TXD_1,     GPIO_103, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(SSP3_TXD,       GPIO_107, 0, OFF,OFF, 0, 0, 8MA),	// Strap 6
    PIN_CFG(SSP_EXTCLK,     GPIO_108, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(SSP5_CLK, 		GPIO_114, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP5_RXD, 		GPIO_115, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(SSP5_TXD, 		GPIO_116, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(SSP6_RXD, 		GPIO_119, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(STAT_1, 		STAT_1,   0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(STAT_2, 		STAT_2,   0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(DIGMIC1_CLK,    GPIO_186, 0, OFF,OFF, 0, 0, 8MA),
    PIN_CFG(DIGMIC1_DQ,     GPIO_187, 0, ON, OFF, 0, 0, 2MA),
    PIN_CFG(DIGMIC2_CLK,    GPIO_188, 0, OFF,OFF, 0, 0, 8MA),
    PIN_CFG(DIGMIC2_DQ,     GPIO_189, 0, ON, OFF, 0, 0, 2MA),

    PIN_CFG(SDIO1_DATA_3,   GPIO_083, 0, ON,  ON, 0, 0, 2MA),
    PIN_CFG(SDIO1_DATA_2,   GPIO_084, 0, ON,  ON, 0, 0, 2MA),
    PIN_CFG(SDIO1_DATA_1,   GPIO_085, 0, ON,  ON, 0, 0, 2MA),
    PIN_CFG(SDIO1_DATA_0,   GPIO_086, 0, ON,  ON, 0, 0, 2MA),
    PIN_CFG(SDIO1_CMD,      GPIO_087, 0, ON,  ON, 0, 0, 2MA),
    PIN_CFG(SDIO1_CLK,      GPIO_088, 0, OFF,OFF, 0, 0, 8MA),
...
static struct pin_config arm3_i2c_config[] = {
    PIN_BSC_CFG(VC_CAM3_SCL, BSC3_SCL, 0x70),
    PIN_BSC_CFG(VC_CAM3_SDA, BSC3_SDA, 0x70),
};
...
static struct pin_config hsi_config[] = {
    ...
    PIN_CFG(SSP4_RXD, ACREADY, 0, ON, OFF, 0, 0, 8MA),
    ...
    PIN_CFG(SSP5_RXD, CAREADY, 0, ON, OFF, 0, 0, 8MA),
...
static struct pin_config ssp3_3_config[] = {
    PIN_CFG(SSP3_FS, SSP3_FS, 0, OFF, OFF, 0, 0, 8MA),
    PIN_CFG(SSP3_CLK, SSP3_CLK, 0, OFF, OFF, 0, 0, 8MA),
    PIN_CFG(SSP3_RXD, SSP3_RXD, 0, ON, OFF, 0, 0, 8MA),
    PIN_CFG(SSP3_TXD, SSP3_TXD, 0, OFF, OFF, 0, 0, 8MA),
...
const struct pin_group pin_config_list[] = {
   ...
   PIN_GROUP_CFG(PG_SSP3_3,     ssp3_3_config,           ARRAY_LEN(ssp3_3_config) ),
};
...
int __init pinmux_board_init(void)
{
    ...
    pinmux_block_init(PG_NAND);		/* NAND and main eMMC are mutually exclusive */
    ...
    // Roku Austin: GPIO05 in the PG_LCD to be used as Reset 
    pinmux_block_init(PG_LCD);
    pinmux_block_init(PG_CLOCKOUT);
    pinmux_block_init(PG_DIGIMIC);
    pinmux_block_init(PG_PMU1);
    pinmux_block_init(PG_USB_HOST1); /* NAND_CEN_1 and NAND_RDY_1 collide - 2nd nand chip won't work with USB host */
    ...
    // Roku Austin: GPIO00 and GPIO01 used as the board ID
...
EXPORT_SYMBOL( pinmux_block_init );
